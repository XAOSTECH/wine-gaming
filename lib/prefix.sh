...
# Install root CAs exactly once; stamp file short-circuits on every subsequent launch.
_ensure_cacerts() {
    ...
    (
        ...
        [ -d "$PROTON_DIR/files/lib64" ] && \
            export LD_LIBRARY_PATH="$PROTON_DIR/files/lib:$PROTON_DIR/files/lib64${LD_LIBRARY_PATH:+:$LD_LIBRARY_PATH}"
        WINETRICKS_LATEST_VERSION_CHECK=enabled winetricks -q --force cacerts >/dev/null 2>&1
    ) && touch "$_stamp" \
