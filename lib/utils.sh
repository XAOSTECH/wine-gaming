...
check_proton() {
    ...
    # A missing bundled kernel32.dll means an on-access AV quarantined it as a false positive; without it Proton dies mid-prefix-build with a raw traceback.
...
# Run "$@" with clamav on-access scanning paused, if it is active. ClamAV false-positives on Proton's bundled Windows PE DLLs and can quarantine them as they are written. No-op when clamonacc isn't running.
run_without_oas() {
